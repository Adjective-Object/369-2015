...
#define read_sp(var) (if (fread(&##var, sizeof(##var), 1, f) != sizeof(##var)) {
	perror("fail on read variable ##var friom super block")
	exit(1);
})
...
ushort inode_size;           // size of inode					(128 in old)
...
ulong  s_uuid;               // uuid of the filesystem
ulong  s_volume_name;        // name of the filesystem, ISO-Latin-1, \0 term'd
...
// Journaling Support
ulong  s_journal_uuid;       // uuid of the journal superblock (?? TODO)
